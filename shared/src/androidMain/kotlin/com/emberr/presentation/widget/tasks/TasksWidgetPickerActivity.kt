...
class TasksWidgetPickerActivity : ComponentActivity() {
    ...
    private fun applySelection(appWidgetId: Int, spaceId: String?) {
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val glanceId = GlanceAppWidgetManager(this@TasksWidgetPickerActivity)
                    .getGlanceIdBy(appWidgetId)

                if (spaceId == null) {
                    clearPinnedSpaceId(this@TasksWidgetPickerActivity, glanceId)
                } else {
                    writePinnedSpaceId(this@TasksWidgetPickerActivity, glanceId, spaceId)
                }

                refreshTaskWidget(this@TasksWidgetPickerActivity, glanceId)
                TasksWidget().update(this@TasksWidgetPickerActivity, glanceId)
                withContext(Dispatchers.Main) { setResult(RESULT_OK, resultIntentFor(appWidgetId)) }
            } catch (cause: Exception) {
                WidgetLog.e("Could not attach a space to the tasks widget", cause)
