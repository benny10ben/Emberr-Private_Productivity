...
class TodayTasksWidgetPickerActivity : ComponentActivity() {
    ...
    private fun applySelection(appWidgetId: Int, spaceId: String?) {
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val glanceId = GlanceAppWidgetManager(this@TodayTasksWidgetPickerActivity)
                    .getGlanceIdBy(appWidgetId)

                if (spaceId == null) {
                    clearPinnedSpaceId(this@TodayTasksWidgetPickerActivity, glanceId)
                } else {
                    writePinnedSpaceId(this@TodayTasksWidgetPickerActivity, glanceId, spaceId)
                }

                refreshTodayTasksWidget(this@TodayTasksWidgetPickerActivity, glanceId)
                TodayTasksWidget().update(this@TodayTasksWidgetPickerActivity, glanceId)
                withContext(Dispatchers.Main) { setResult(RESULT_OK, resultIntentFor(appWidgetId)) }
            } catch (cause: Exception) {
                WidgetLog.e("Could not attach a space to the today tasks widget", cause)
