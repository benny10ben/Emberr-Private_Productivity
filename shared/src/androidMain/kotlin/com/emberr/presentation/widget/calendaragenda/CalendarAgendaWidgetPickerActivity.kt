...
class CalendarAgendaWidgetPickerActivity : ComponentActivity() {
    ...
    private fun applySelection(appWidgetId: Int, spaceId: String?) {
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val glanceId = GlanceAppWidgetManager(this@CalendarAgendaWidgetPickerActivity)
                    .getGlanceIdBy(appWidgetId)

                if (spaceId == null) {
                    clearPinnedSpaceId(this@CalendarAgendaWidgetPickerActivity, glanceId)
                } else {
                    writePinnedSpaceId(this@CalendarAgendaWidgetPickerActivity, glanceId, spaceId)
                }

                refreshCalendarAgendaWidget(this@CalendarAgendaWidgetPickerActivity, glanceId)
                CalendarAgendaWidget().update(this@CalendarAgendaWidgetPickerActivity, glanceId)
                withContext(Dispatchers.Main) { setResult(RESULT_OK, resultIntentFor(appWidgetId)) }
            } catch (cause: Exception) {
                WidgetLog.e("Could not attach a space to the calendar agenda widget", cause)
