...
class NoteListWidgetPickerActivity : ComponentActivity() {
    ...
    private fun applySelection(appWidgetId: Int, spaceId: String?) {
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val glanceId = GlanceAppWidgetManager(this@NoteListWidgetPickerActivity)
                    .getGlanceIdBy(appWidgetId)

                if (spaceId == null) {
                    clearPinnedSpaceId(this@NoteListWidgetPickerActivity, glanceId)
                } else {
                    writePinnedSpaceId(this@NoteListWidgetPickerActivity, glanceId, spaceId)
                }

                refreshNoteListWidget(this@NoteListWidgetPickerActivity, glanceId)
                NoteListWidget().update(this@NoteListWidgetPickerActivity, glanceId)
                withContext(Dispatchers.Main) { setResult(RESULT_OK, resultIntentFor(appWidgetId)) }
            } catch (cause: Exception) {
                WidgetLog.e("Could not attach a space to the note list widget", cause)
