...
class CalendarWidgetPickerActivity : ComponentActivity() {
    ...
    private fun applySelection(appWidgetId: Int, spaceId: String?) {
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val glanceId = GlanceAppWidgetManager(this@CalendarWidgetPickerActivity)
                    .getGlanceIdBy(appWidgetId)

                if (spaceId == null) {
                    clearPinnedSpaceId(this@CalendarWidgetPickerActivity, glanceId)
                } else {
                    writePinnedSpaceId(this@CalendarWidgetPickerActivity, glanceId, spaceId)
                }

                refreshCalendarWidget(this@CalendarWidgetPickerActivity, glanceId)
                CalendarWidget().update(this@CalendarWidgetPickerActivity, glanceId)
                withContext(Dispatchers.Main) { setResult(RESULT_OK, resultIntentFor(appWidgetId)) }
            } catch (cause: Exception) {
                WidgetLog.e("Could not attach a space to the calendar widget", cause)
